...
function createComment(url, request) {
	...
	if(requestComment && requestComment.body && requestComment.articleId && requestComment.username && database.users[requestComment.username] && database.articles[requestComment.articleId]) {
		...
		// add comment's id to username's list of comments
		database.users[comment.username].commentIds.push(comment.id);
		// add comment's id to article so it is associated with article
...
function deleteComment(url, request) {
	...
	if(savedComment) {
		//clears out comment in comment database
		database.comments[id] = null;
		// finds comment id in user database and removes it there
		const userCommentIds = database.users[savedComment.username].commentIds;
		userCommentIds.splice(userCommentIds.indexOf(id), 1);
		// finds comment id in article database and removes it from comment
